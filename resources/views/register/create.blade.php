            ...
            <ol class="inline-flex items-center space-x-1 md:space-x-3 gap-1">
                ...
                <li>
                    <div class="flex items-center gap-1">
                        ...
                        <a
                            class="ml-1 text-sm font-medium text-[#03a9f4]  md:ml-2 dark:text-gray-400 dark:hover:text-white">Create
                            an account</a>
                    </div>
                </li>
            </ol>
...
<div class="md:container mx-auto px-7">
    ...
    <h1 class="text-xs mb-1 text-gray-500">Create account and explore the world</h1>
    ...
    <section class="md:container  px-7 mt-5  flex items-center mx-auto justify-center bg-2b2d42">
        ...
        <div class="md:container bg-gray-100 mx-auto flex max-w-full p-5 items-center text-black  ">
            ...
            <div class="md:w-1/2 px-8 md:px-16 ">
                ...
                <p class="mt-4 md:text-xs grid text-center text-[#002D74]">If you are not a member, register now</p>

                <form action="/register" class="flex flex-col gap-4 mt-2" method="POST" id="loginForm">
                    @csrf
                    <input class="p-2 px-7 rounded-xl border border-blue-400 w-full" id="name" type="name"
                        name="name" placeholder="Name" value="{{ old('name') }}" required>
                    @error('name')
                        <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                    @enderror
                    <input class="p-2 px-7 rounded-xl border border-blue-400 w-full" id="username" type="username"
                        name="username" placeholder="Username" value="{{ old('username') }}"required>
                    @error('username')
                        <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                    @enderror
                    <input class="p-2 px-7 rounded-xl border border-blue-400 w-full" id="email" type="email"
                        name="email" placeholder="email" value="{{ old('email') }} "required>
                    @error('email')
                        <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                    @enderror
                    <div class="relative">
                        <input class="p-2 px-7 rounded-xl border border-blue-400 w-full" id="password"
                            type="password" name="password" placeholder="Password" required>
                        @error('password')
                            <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                        @enderror
                        <svg id="togglePassword" xmlns="http://www.w3.org/2000/svg" width="16" height="16"
                            fill="gray" class="cursor-pointer bi bi-eye absolute top-1/2 right-3 -translate-y-1/2"
                            viewBox="0 0 16 16">
                            <path
                    ...
                    <button type="submit" name="submit" id="submit"
                        class="bg-black rounded-md text-white py-2 hover:scale-105 duration-300 mt-5">Register</button>
                </form>


                <div class="mt-3 text-xs flex justify-between items-center text-[#002D74] ">
                    <p>Already have an account?</p>
                    <button class="py-2 px-5 bg-white border rounded-xl hover:scale-110 duration-300 "><a
                            href="{{ URL::to('login') }}">Login</a> </button>
                </div>
            </div>
